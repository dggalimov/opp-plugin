...
# 1. Репозиторий плагина: клон или обновление. Существующая папка без .git (ручная
#    раскладка прошлых версий) — в бэкап рядом, на её место — чистый клон.
if [ -d "$INSTALL_DIR/.git" ]; then
    git -C "$INSTALL_DIR" pull --ff-only -q
    echo "  репозиторий обновлён: $INSTALL_DIR"
else
    if [ -d "$INSTALL_DIR" ]; then
        mv "$INSTALL_DIR" "$INSTALL_DIR.бэкап-$(date +%Y%m%d-%H%M%S)"
        echo "  прежняя раскладка (не git) отложена в $INSTALL_DIR.бэкап-*"
    fi
