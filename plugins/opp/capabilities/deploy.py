...
Соответствие «песочным часам»: Источники (сырьё) → Проектная память (факты по уровням) → Документы.
"""
...
def _overview_text(name: str, channels: list[str]) -> str:
    return (
        ...
        f"Структура: **Источники** (сырьё) → **Проектная память** (факты по 11 уровням каркаса) → "
        f"**Документы** (результат).\n\n"
...
def deploy_workspace(target, channels=None) -> list[Path]:
    ...
    # отсюда; технический слой узлов-источников — в «Источники/», пользователь туда не ходит.
    mkdir(target / "Входные материалы")
    mkdir(target / "Встречи")

    sources = target / "Источники"
    mkdir(sources)
    for ch in channels:
