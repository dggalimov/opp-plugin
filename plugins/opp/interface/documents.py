...
`<проект>/Документы/<id>.md`) и «Собрать docx» (`capabilities.render.docx.render_docx` поверх
только что пересобранного md, в тот же `<проект>/Документы/`).
...
from capabilities.render.engine import DOCUMENTS_DIR, load_config, render_markdown
...
def _save(doc_id: str, _content: str, project):
    ...
    Id с суффиксом «::docx» — кнопка «Собрать docx»: md пересобирается тем же путём, затем
    поверх него — docx-представление (тема — как в cli.py::cmd_render, без титула клиента/даты —
    их в окне не ввести; заголовок docx берётся из уже загруженного эффективного конфига, тем
    самым согласован с проектным шаблоном, если он есть)."""
    if project is None:
    ...
    (out_dir / f"{doc_id}.md").write_text(rendered_md, encoding="utf-8")
    if not want_docx:
    ...
    title = (cfg.get("документ") or {}).get("название", doc_id)
    try:
        docx_path = render_docx(rendered_md, out_dir / f"{doc_id}.docx",
                                 project=str(project), document_title=title)
    except Exception as exc:  # noqa: BLE001
